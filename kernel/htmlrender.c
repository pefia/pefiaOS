 ...
 *                  |--> collect <style>/<link rel=stylesheet> --css--> rule list
 *                  |--> run <script> --js--> mutate DOM
 *                  '--> tree walk + css_cascade --> flowed spans --> framebuffer
 *
 * Layout produces one flat run of "spans" (text run / image / horizontal
 * rule) in document order. That's a deliberately dumb representation - no
 * box tree, no reflow beyond initial flow - but it makes painting and link
 * hit-testing a single linear scan, which is what we actually need for a
 * scrollable page. Every span's color/weight/scale/alignment and block
 * backgrounds come from the cascaded ComputedStyle of the element that
 * produced it. Decoded images are cached per-URL so scrolling or resizing
 * the window doesn't refetch anything.
 */
...
/* Per-URL cache of the full-resolution decoded bitmap, kept across
...
static void emit_run(const char *s, int len)
{
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (c == '&') {
            ...
            else if (wl < (int)sizeof(g_word_buf)-1) g_word_buf[wl++] = (char)ch;
            continue;
        }
        if (c == ' ' || c == '\t' || c == '\r' || (c == '\n' && !g_pre)) {
        ...
        if (wl < (int)sizeof(g_word_buf)-1) g_word_buf[wl++] = c;
    }
...
/* Fetches (if not already cached) and decodes the full-resolution bitmap for url. */
static ImgCacheEnt *img_cache_get(const char *url)
{
    ...
    /* grab dimensions before bmp_free() zeroes the struct out */
    int bw = bmp.width, bh = bmp.height, bstride = bmp.stride;
    if (bw <= 0 || bh <= 0) { bmp_free(&bmp); return 0; }
...
static void emit_image(const char *src, const char *alt, int attr_w, int attr_h)
{
    ...
    if (g_img_mem + bytes > IMG_MEM_CAP) { emit_img_alt_text(alt); return; }
    uint32_t *scaled = (uint32_t *)kmalloc((int)bytes);
    if (!scaled) { emit_img_alt_text(alt); return; }
 ...
 * should be visited next, 0 if the subtree is fully handled here (text
 * nodes, <br>, <hr>, <img> - none of those have DOM children we care about). */
...
static void relayout(int width)
{
    ...
    g_link = -1; g_indent = 0; g_pre = 0; g_sf_top = 0; g_line_start = 0;

    DomNode *root = dom_parse_document(g_page, g_page_len);
    compute_title(root);
    collect_stylesheets(root);
    run_scripts(root);
    compute_title(root);   /* scripts may have replaced <body>/its children */
        ...
        if (s->img >= 0 && s->img < g_nimages) {
            ...
            for (int iy = 0; iy < im->h; iy++) {
                ...
                for (int ix = 0; ix < im->w; ix++) {
                    uint32_t p = row[ix];
                    if ((p >> 24) < 16) continue;          /* mostly-transparent: skip */
